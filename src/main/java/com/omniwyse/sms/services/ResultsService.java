...
public class ResultsService {
    ...
    private static final String GETSUB ="SELECT DISTINCT str.classid,str.studentid AS studentid,st.name AS studentname,str.testid,tt.testtype,str.subjectid,ts.maxmarks,str.marks,stble.status FROM student_testresult str LEFT JOIN classroom_testresult cltrs ON str.testid = cltrs.testid LEFT JOIN test_create tc ON tc.id = str.testid LEFT JOIN grade_subjects gs ON gs.gradeid = tc.gradeid LEFT JOIN students st ON str.studentid = st.id LEFT JOIN test_syllabus ts ON ts.subjectid = gs.subjectid AND str.testid = ts.testid LEFT JOIN test_type tt ON tt.id = tc.testtypeid JOIN status_table stble ON stble.id = tc.statusid WHERE str.testid = ? AND str.subjectid = ? AND str.classid = ? AND ts.maxmarks IS NOT NULL";
    ...
    public MainResultsTransferObject viewResults(ResultsTransferObject resultstransferobject,long tenantId) {
        ...
        List<ResultsTransferObject> resulttransferlist = db.sql("select distinct cltrs.studentid as studentid,st.name,tt.testtype,cltrs.resultorgrade,cltrs.testid,statustb.status from classroom_testresult cltrs "
                        ...
                        + " left join test_syllabus ts on ts.subjectid = gs.subjectid left join test_type tt on tt.id =tc.testtypeid join status_table statustb on tc.statusid = statustb.id where cltrs.testid=? and gs.gradeid=? and cltrs.classid=?",
                        testcreateid, gradeid,classid).results(ResultsTransferObject.class);
...
//            
        			resulttransferobj = db.sql(GETSUB, testcreateid, subjectid, classid).results(ResultsTransferObject.class);
