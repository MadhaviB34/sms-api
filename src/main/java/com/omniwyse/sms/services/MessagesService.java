		...
		for (MessagesDetails replymessage : replymessages) {
			if (replymessage.getSentflag().equals("T")) {
				List<Teachers> teachers = 
						db.where("id=?", replymessage.getSenderid()).results(Teachers.class);
				if(!teachers.isEmpty()) {
					replymessage.setSendername(teachers.get(0).getTeachername());
				}
				/*
				 * replymessage.setSendername( db.where("id=?",
				 * replymessage.getSenderid()).results(Teachers.class).get(0).getTeachername());
				 */
