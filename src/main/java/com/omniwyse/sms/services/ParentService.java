	...
	public MainResultsTransferObject studentsMarks(long studentid, long gradeid, long classid, long testcreateid,
		 ...
		 db = retrieve.getDatabase(tenantId);
	       ...
	       List<ResultsTransferObject> resulttransferlist = db.sql("select distinct cltrs.studentid as studentid,st.name,tt.testtype,cltrs.resultorgrade from classroom_testresult cltrs "
	        ...
	        List<StudentSubjectMarks> marks = resulttransferlist.get(0).getStudentsubjectmarks();
	       
	        //To set the marks each subjectwise
	        for(StudentSubjectMarks studentSubMarks : subjectlist) {
	        	for(StudentSubjectMarks marksSubWise : marks) {
	        		if(marksSubWise.getSubjectname().equals(studentSubMarks.getSubjectname())) {
	        			studentSubMarks.setMarks(marksSubWise.getMarks());
	        		}
	        	}
	        }
